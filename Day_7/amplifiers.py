...
def input_intcode(input_list, index, amp_values=None):
    if amp_values is None:
        to_input = int(input("Your integer input please: "))
    else:
        to_input = amp_values.pop(0)
...
def output_intcode(input_list, index, parameter_modes):
    ...
    if int(parameter_modes):
        return input_list[index + 1], index + 2
    else:
        return input_list[int(input_list[index + 1])], index + 2
...
def run_intcode_instructions(input_list, amp_values=None, index=0):
    while True:
        ...
        if opcode == 1:
            ...
            input_intcode(input_list, index, amp_values)
            index += 2
        elif opcode == 4:
            return output_intcode(input_list, index, parameter_modes)
            index += 2
        elif opcode == 5:
            ...
            return "Opcode 99: exiting!", index
        else:
            raise ValueError("Incorrect instruction code found!", index, opcode)


def find_best_output(inputs, is_part_two=False):
    best_output = 0
    if is_part_two:
        permutation_list = [i for i in permutations(range(5, 10))]
    else:
        permutation_list = [i for i in permutations(range(5))]
    for permutation in permutation_list:
        command_list = []
        indexes = []
        for i in range(5):
            if i == 0:
                command_list.append(deepcopy(inputs))
                output, new_index = run_intcode_instructions(command_list[i], [permutation[i], 0])
                indexes.append(new_index)
            else:
                command_list.append(deepcopy(inputs))
                output, new_index = run_intcode_instructions(command_list[i], [permutation[i], output])
                indexes.append(new_index)
        if is_part_two:
            running = True
            while running:
                for i in range(5):
                    new_output, new_index = run_intcode_instructions(command_list[i], [output], indexes[i])
                    indexes[i] = new_index
                    if type(new_output) is not int:
                        running = False
                    else:
                        output = new_output
...
if __name__ == '__main__':
    ...
    best_output = find_best_output(inputs, True)
